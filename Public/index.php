...
function getAvatarFromDomain() {
  $h = filter_input(INPUT_SERVER, "HTTP_HOST");
  $ipos = mb_stripos($h, ".");
  $sd = substr($h, 0, $ipos);
  return $sd;
}
...
switch ($url) {
  case "action":
    ...
    break;
  case "rw":
    define("SCRIPT_NAME", "homerwproxy");
    define("SCRIPT_FILENAME", "homerwproxy.php");
    break;
  case "":
  case "home":
  ...
  case "img":
    ...
    if ($pic === APP_DEF_PROFILE_PIC) {
      ...
      $picPath = $BLOG_PATH . DIRECTORY_SEPARATOR . $pic;
    }  
  ...
  case "file":
    ...
    break;    
  case "genmenu":
    define("SCRIPT_NAME", "genmenu");
    define("SCRIPT_FILENAME", "genmenu.php");   
    define("AVATAR_NAME", getAvatarFromDomain());
    break;   
  case "man":
    define("SCRIPT_NAME", "man1");
    define("SCRIPT_FILENAME", "man1.php");  
    define("AVATAR_NAME", getAvatarFromDomain());
    break;     
  case "man2":
    define("SCRIPT_NAME", "man2");
    define("SCRIPT_FILENAME", "man2.php");   
    define("AVATAR_NAME", getAvatarFromDomain());
    break;       
  case "qrcode":
    define("SCRIPT_NAME", "qrcode");
    define("SCRIPT_FILENAME", "qrcode.php");   
    break;
  default:
    define("SCRIPT_NAME", "home");
    define("SCRIPT_FILENAME", "home.php");   
    ...
    if (mb_stripos($url, "-") === false) {
      ...
      define("ACTUAL_DISCOUNT", 0);   
    } else {
      define("AVATAR_NAME", explode("-", $url)[0]); 
      define("AVATAR_NAME_WITH_DISC", $url);
      $discount = (int)substr(explode("-", $url)[1],1,2); 
      define("ACTUAL_DISCOUNT", $discount);   
    }
